...
const DateRangeLabel = props => {
  ...
  const time = format.time || 'HH:mm'
  const dm = `${d} ${m}`
  const dmy = `${d} ${m} ${y}`

  const dt = `${d} ${time}`
  const dtmy = `${d} ${time} ${m} ${y}`
  ...
  if(start.d === end.d && start.m === end.m && start.y === end.y){
    rangeLabel = `${startDate.format(dt)} - ${endDate.format(time)} ${startDate.format(dmy)}`
  } else if (start.m === end.m && start.y === end.y) {
    rangeLabel = `${startDate.format(dt)} - ${endDate.format(time)} ${endDate.format(dmy)}`
